...
/**
 * CheckIn counter model
 *
 * @package pt_conference
 * @subpackage Domain/Model/Counter/
 * @author Daniel Lienert <lienert@punkt.de>
 */
class  Tx_PtConference_Domain_Model_Counter_CheckInCounter extends Tx_PtConference_Domain_Model_Counter_AbstractCounter {
	
	/**
	 * Get the scanned tickets from scannerCache 
	 * and set the property checkedIn of the scanned tickets
	 */
	public function submitScannerList() {
		...
		$this->markTicketObjectsAsCheckedIn($persDataObjects);
	...
	protected function markTicketObjectsAsCheckedIn(array $persDataObjects) {
		foreach($persDataObjects as $persDataObject) {
			$persDataObject->setCheckedin(1);
	...
	protected function setRestrictedTicketCodes() {
		$ticketObjects = $this->persDataRepository->findAllByCheckedin(1);
